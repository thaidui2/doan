...
checkPermissionRedirect('role_edit');
...
$role_stmt = $conn->prepare("SELECT * FROM roles WHERE id_role = ?");
...
$role_permissions = [];
$permission_query = $conn->prepare("SELECT id_permission FROM role_permissions WHERE id_role = ?");
$permission_query->bind_param("i", $role_id);
$permission_query->execute();
$permission_result = $permission_query->get_result();

while ($row = $permission_result->fetch_assoc()) {
    $role_permissions[] = $row['id_permission'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($role_name)) {
        ...
        $check_stmt = $conn->prepare("SELECT id_role FROM roles WHERE ten_role = ? AND id_role != ?");
        ...
        if ($check_result->num_rows > 0) {
            ...
            try {
                // Update role
                $update_stmt = $conn->prepare("UPDATE roles SET ten_role = ?, mo_ta = ? WHERE id_role = ?");
                $update_stmt->bind_param("ssi", $role_name, $role_description, $role_id);
                $update_stmt->execute();
                
                // Delete all current permissions for this role
                $delete_perms = $conn->prepare("DELETE FROM role_permissions WHERE id_role = ?");
                ...
                // Rollback on error
                $conn->rollback();
                $_SESSION['error_message'] = 'Lỗi khi cập nhật vai trò: ' . $e->getMessage();
            }
        }
    }
}
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Chỉnh sửa vai trò</h1>
        ...
        <div class="card-body">
            <form method="post" action="">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="role_name" name="role_name" value="<?php echo htmlspecialchars($role['ten_role']); ?>" required>
                </div>
                
                <div class="mb-3">
                    <label for="role_description" class="form-label">Mô tả</label>
                    <textarea class="form-control" id="role_description" name="role_description" rows="3"><?php echo htmlspecialchars($role['mo_ta'] ?? ''); ?></textarea>
                </div>
                                                    ...
                                                    <?php foreach ($permissions as $permission): ?>
                                                        <div class="col-lg-4 col-md-6 mb-2">
                                                            <div class="form-check">
                                                                <input class="form-check-input permission-checkbox group-<?php echo $index; ?>" type="checkbox" name="permissions[]" value="<?php echo $permission['id_permission']; ?>" id="perm_<?php echo $permission['id_permission']; ?>" <?php echo in_array($permission['id_permission'], $role_permissions) ? 'checked' : ''; ?>>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Expand accordions that have checked items
        const expandAccordionsWithCheckedItems = function() {
            document.querySelectorAll(".accordion-item").forEach(function(accordionItem) {
                const checkedItems = accordionItem.querySelectorAll("input[type=checkbox]:checked").length;
                if (checkedItems > 0) {
        ...
        // Call function after a short delay to ensure DOM is fully loaded
        setTimeout(expandAccordionsWithCheckedItems, 100);
    });
</script>
';
